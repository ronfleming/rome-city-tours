<template>
  <div id="create">
    <form class="new-itinerary-form" v-on:submit.prevent="saveItinerary">
      <h2 class="test">Create Itinerary</h2>
      <input
        class="name-input"
        type="text"
      ...
      <textarea placeholder="Notes" v-model="formItinerary.notes" />
      <br /><br />
      <button class="example">Save Itinerary</button>
    </form>
    <div id="landmarkinfo">
...
export default {
  ...
  methods: {
    ...
    saveItinerary() {
      ...
      //end test
      tourService.addItinerary(this.formItinerary).then((response) => {
  ...
  created() {
    ...
    tourService.getLandmarkByLandmarkID(1).then((response) => {
      this.landmark = response.data;
      this.location = { lat: this.landmark.lat, lng: this.landmark.lng };
    });
  },
